...
def __dirinfo(part):
    ...
    while head:
        ...
        if tail == part:
            checkfile = os.path.join(partdir, os.pardir, project_dir, '__init__.py')
...
def __clone():
    ...
    if not this_dir.endswith(join('alex','tools')):
        raise EnvironmentError("can only clone master version "
                               "'%s'" % join(pypydir, 'tools',_myname))
    ...
    def sync_walker(arg, dirname, fnames):
        if _myname in fnames:
            ...
            try:
                ...
                f.close()
    s = open(join(pypydir, 'tools', _myname), 'rb').read()
...
project_dir = 'wmt16'
pypydir, this_dir = __dirinfo(project_dir)
